...
import events as e
from .callbacks import state_to_features, act_features, Q_TABLE_FILE, ACTIONS
...
SCORE_FILE = "less-dense_versus_collectors.pkl"
WINS_FILE = "less-dense_versus_collectors_wins.pkl"
OTHERS_SCORE_FILE = "less-dense_versus_collectors_scores.pkl"
...
def update_q_values(self):
    ...
    while self.transitions:
        ...
        if next_state is None:
            ...
            next_action = act_features(self, next_state)
            next_q_value = next_q_values.get(next_action, 0.0)
            new_reward = reward + self.gamma * next_q_value
...
def game_events_occurred(self, old_game_state: dict, self_action: str, new_game_state: dict, events: List[str]):
    ...
    if e.BOMB_DROPPED in events and old_features[15] > 0:
        events.append(DROPPED_BOMB_AT_CRATE)
    if e.BOMB_DROPPED in events and old_features[16] == 1:
    ...
    if e.MOVED_RIGHT in events and old_features[10] == 1:
        events.append(MOVE_CLOSER_TO_EXPLOSION)
    if e.MOVED_UP in events and old_features[11] == 1:
        events.append(MOVE_CLOSER_TO_EXPLOSION)
    if e.MOVED_DOWN in events and old_features[12] == 1:
        events.append(MOVE_CLOSER_TO_EXPLOSION)
    if e.WAITED in events and old_features[13] == 1:
        events.append(MOVE_CLOSER_TO_EXPLOSION)
